...
/* ------------------------------------------------------------------ */
/*  Current term / year helpers                                        */
/* ------------------------------------------------------------------ */

export async function getCurrentTerm(session) {
  requireSchoolAdmin(session);
  const schoolId = session.school.id;
  try {
    /* Fetch all terms for the school and find the current one client-side —
       avoids needing a composite index on (school_id + is_current). */
    const snap = await col("terms").where("school_id", "==", schoolId).get();
    const terms = docsFrom(snap);
    /* is_current may be stored as 1, true, or "1" depending on how the
       record was created — treat any of those markers as current. */
    const currentTerm = terms.find((t) =>
      Number(t.is_current) === 1 || t.is_current === true || String(t.is_current) === "1"
    ) || null;
    return { success: true, term: currentTerm };
  } catch (e) {
    console.error("Error fetching current term:", e);
    return { success: true, term: null };
  }
}

export async function getCurrentAcademicYear(session) {
  requireSchoolAdmin(session);
  const schoolId = session.school.id;
  try {
    /* Fetch all academic years for the school and find the current one
       client-side — avoids needing a composite index. */
    const snap = await col("academic_years").where("school_id", "==", schoolId).get();
    const years = docsFrom(snap);
    const currentYear = years.find((y) =>
      Number(y.is_current) === 1 || y.is_current === true || String(y.is_current) === "1"
    ) || null;
