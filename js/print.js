...
function printHeadMarkup(sName, sCode, title, sHead = "") {
  return `
    <div class="print-head">
      <div class="print-brand">
        ...
        <div>
          ...
          ${sHead ? `<div class="print-brand-head" style="font-size:10px;color:#1E3A8A;font-weight:600;margin-top:2px;">Head: ${escapeHtml(sHead)}</div>` : ""}
        </div>
      </div>
      <div>
...
export function printHTML({ title = "ShuleSmart", content = "", school = null } = {}) {
  ...
  const sHead = ctx.head_name ? ctx.head_name.trim() : "";
...
<style>
  *{box-sizing:border-box;} html,body{margin:0;padding:0;}
  ...
  .report-cards .report-card{break-inside:avoid;break-after:page;page-break-inside:avoid;page-break-after:always;page-break-before:always;}
  .report-cards .report-card:last-child{break-after:auto;page-break-after:auto;}
</style>
</head>
<body>
  <div class="print-doc">
    ${content.startsWith('<div class="report-cards">') || content.includes('<div class="print-head">') ? '' : '<div class="print-head"><div class="print-brand"><div class="print-mark">' + escapeHtml(initials(sName)) + '</div><div><div class="print-brand-name">' + escapeHtml(sName) + '</div>' + (sCode ? '<div class="print-brand-code">' + escapeHtml(sCode) + '</div>' : "") + (sHead ? '<div class="print-brand-head" style="font-size:10px;color:#1E3A8A;font-weight:600;margin-top:2px;">Head: ' + escapeHtml(sHead) + '</div>' : "") + '</div></div><div><div class="print-title">' + escapeHtml(title) + '</div><div class="print-sub">Generated ' + escapeHtml(formatDateTime(new Date().toISOString())) + '</div></div></div>'}
    ${content}
  </div>
</body>
</html>`;
...
export function buildReceiptContent({ student = {}, payment = {}, balance = 0, appliedToFees = 0, creditCarried = 0, status = "ISSUED", reprint = null } = {}) {
  ...
  <div class="print-grid">
    ...
    <div class="print-field"><span class="k">Recorded On</span><span class="v">${escapeHtml(formatDateTime(payment.created_at || payment.payment_date))}</span></div>
  </div>
...
export function buildStatementContent({ student = {}, lines = [], totals = {}, school = {}, period = {} } = {}) {
  ...
  const clsShort = (student.class_name || "")
    ...
    ? lines.map((l) => `
      <tr>
        ...
        <td>
          ...
          ${clsShort ? `<span class="print-badge" title="${escapeHtml(student.class_name || "")}">${escapeHtml(clsShort)}</span>` : ""}
 ...
 * remark is stored on the result. Offers a VARIETY of phrases per
 * performance band (chosen deterministically, seeded by the student's
 * name/id) so different students — and even siblings of similar standing —
 * don't all receive identical wording. The same student always sees the
 * same remark across re-prints. */
const REMARK_BANDS = {
  excellent: [
    "Excellent performance — keep up the outstanding work.",
    "Brilliant results! You are setting a great example for the class.",
    "Outstanding effort. Keep aiming for the very best.",
    "Exceptional work across the board. Continue as you are!",
    "A superb performance. Your discipline and focus are paying off."
  ],
  veryGood: [
    "Very good. Continue to apply consistent effort.",
    "Good progress and solid results. Push a little more to reach excellent.",
    "Well done — you are on the right track. Keep it up!",
    "A strong performance. Stay consistent and you will shine.",
    "Great improvement. Keep building on this success."
  ],
  good: [
    "Good. There is room for improvement with more practice.",
    "You are doing well. Consistent revision will raise your scores.",
    "A fair foundation has been laid. Dedicate more time to your weakest subjects.",
    "Keep going — regular practice will turn good into great.",
    "Satisfactory effort. Aim higher and put in a bit more work."
  ],
  fair: [
    "Fair. More effort and regular revision are needed.",
    "There is noticeable room for improvement. Focus on your studies daily.",
    "Average performance. Seek help early and revise consistently.",
    "You can do better with extra practice and better concentration.",
    "A pass, but more self-drive would bring a real step forward."
  ],
  weak: [
    "Weak. Requires extra support and focused revision.",
    "Performance is below expectation. Extra guidance and daily revision are required.",
    "Hard work is needed, especially in the weaker subjects. Do not give up.",
    "Falling behind — please arrange for extra help and revise thoroughly.",
    "Effort has been minimal. More commitment and support are essential."
  ]
};

function pickRemark(band, seed) {
  const pool = REMARK_BANDS[band] || REMARK_BANDS.good;
  /* Deterministic "random" index from the seed string — stable across prints. */
  let h = 0;
...
function defaultRemark(average, grade, useGrades, seed) {
  let band;
  if (useGrades) {
    const g = String(grade || "").toUpperCase();
    if (g === "A") band = "excellent";
    else if (g === "B") band = "veryGood";
    else if (g === "C") band = "good";
    else if (g === "D") band = "fair";
    else if (g === "E") band = "weak";
    else band = "good";
  } else {
    const a = Number(average);
    if (a >= 80) band = "excellent";
    else if (a >= 65) band = "veryGood";
    else if (a >= 50) band = "good";
    else if (a >= 35) band = "fair";
    else band = "weak";
  }
  return pickRemark(band, seed);
}
...
/* detail.school carries the school context so each page can render the
   branded letterhead (school name / code) just like the first page. */
export function buildSingleReportCard(student = {}, detail = {}) {
  ...
  /* School branding for the header */
  const sName = detail.school ? ((detail.school.name || "").trim() || "ShuleSmart") : "ShuleSmart";
  const sCode = detail.school ? ((detail.school.school_code || detail.school.code || "").trim()) : "";
  const sHead = detail.school ? ((detail.school.head_name || "").trim()) : "";
  ...
  const remark = defaultRemark(student.average, student.grade, useGrades, `${student.name || ""}|${student.admission_number || student.id || ""}`);

          return printHeadMarkup(sName, sCode, "Student Report Card", sHead) + `
    ...
    <div class="print-remark" style="font-size:12px;">${escapeHtml(remark)}</div>
      ...
      ${sHead ? `<div><div style="font-weight:700;">Headteacher</div><div class="print-head-name" style="font-weight:700;color:#1E3A8A;margin-top:26px;font-size:13px;">${escapeHtml(sHead)}</div></div>` : `<div><div style="font-weight:700;">Headteacher</div><div class="line"></div></div>`}
