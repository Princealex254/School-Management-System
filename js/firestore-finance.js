...
async function studentLedger(schoolId, studentId) {
  /* Two-field (composite) query is the fast path but needs a deployed
     composite index. Fall back to the single-field school query (which uses
     the automatic index and always works) and filter in memory — so an
     undeployed composite index can never silently zero-out a student's
     ledger and make the View overlay show "—" / 0 for every financial field. */
  let snap = null;
  try {
    snap = await col("student_ledger")
      .where("school_id", "==", schoolId)
      .where("student_id", "==", studentId)
      .get();
  } catch {
    try {
      const all = await col("student_ledger")
        .where("school_id", "==", schoolId)
        .limit(8000)
        .get();
      const out = [];
      all.forEach((d) => { if (d.data().student_id === studentId) out.push(snapDoc(d)); });
      return out;
    } catch { return []; }
  }
...
/** FIFO-allocate a payment amount across a student's open charges (oldest
 *  first). Returns the charge allocations, the total applied to fees, and
 *  any remainder carried as credit.
 *
 *  Shared by:
 *   - createPayment     (writes the authoritative applied/credit at record time)
 *   - getPaymentFinancials fallback (a historical payment with no recorded
 *     allocations is never shown as "Applied 0 / Credit full" when the
 *     student demonstrably had outstanding charges — it re-derives how much
 *     of the payment those charges consumed). */
function fifoAllocate(ledger, amount) {
  const netByReference = {};
  for (const l of ledger) {
    if (l.entry_type !== "CHARGE") continue;
  ...
  return { allocations, appliedTotal, creditCarried };
}
...
export async function getPaymentFinancials(session, id) {
  ...
  /* Applied to Fees = what this payment cleared of outstanding charges.
     Authoritative source priority:
       1. stored on the payment doc (createPayment persists the FIFO result),
       2. sum of this payment's `payment_allocations`,
       3. re-derive from the ledger via fifoAllocate — a payment recorded
          before/without allocations is never shown as "Applied 0 / Credit
          full" when the student demonstrably had outstanding charges. */
  let appliedTotal = null;
  let hadAllocationsRow = false;
  let hadStoredValue = p.applied_to_fees != null;
  try {
    if (hadStoredValue) {
      appliedTotal = Number(p.applied_to_fees);
    } else {
      let allocSum = 0;
      const asnap = await col("payment_allocations").where("payment_id", "==", id).get();
      asnap.forEach((d) => { allocSum += Number(d.data().amount || 0); });
      hadAllocationsRow = !asnap.empty;
      appliedTotal = allocSum > 0 ? allocSum : null;
    }
  } catch { /* falls through to derive */ }
  ...
  const needsDerive = appliedTotal == null;
  if (needsDerive) {
    appliedTotal = fifoAllocate(ledger, amount).appliedTotal;
  }
  const applied = Math.max(0, Math.min(Math.round(appliedTotal * 100) / 100, amount));
  const credit = Math.max(0, Math.round((amount - applied) * 100) / 100);

  /* Backfill — take care of ALREADY-EXISTING payments. Older records predate
     the stored applied_to_fees/credit_carried fields and often have no
     payment_allocations row, so this view is the natural place to persist the
     derived truth once. Safe & idempotent: only writes when the value was
     missing, never overwrites an authoritative allocation. */
  if (needsDerive || !hadStoredValue) {
    try {
      await col("payments").doc(id).set(
        { applied_to_fees: applied, credit_carried: credit },
        { merge: true }
      );
      if (!hadAllocationsRow && applied > 0) {
    ...
    } catch { /* read-only or offline — display-only success is fine */ }
  }
...
export async function createPayment(session, body) {
  ...
  const { allocations, appliedTotal, creditCarried } = fifoAllocate(ledger, amount);
  ...
  /* Enrich the create response with rendered student details (name /
     admission / class) so the "view" overlay that opens immediately after
     recording shows everything at once — no need to close & reopen it. */
  const card = await studentCard(studentId) || {};
  return {
    success: true,
    ...
    payment: {
      ...
      receipt_no,
      amount, method, reference, payment_date: payDate, status: "COMPLETED",
      recorded_by: session.user.display_name,
      notes,
      created_at: createdAt
    },
...
export async function generateFeeStructureCharges(session, id) {
  ...
  /* is_current may be stored as 1, true, or "1" depending on how the term
     was created — treat any truthy "current" marker as active. */
  const isTermCurrent = Number(term.is_current) === 1 || term.is_current === true || String(term.is_current) === "1";
  if (!isTermCurrent) {
    throw new Error("You can only generate charges for the current, active term. This structure is set to a term that is not currently active. Go to Terms, edit this term and tick 'Current Term'.");
  }
  ...
  const isYearCurrent = Number(ayDoc.data().is_current) === 1 || ayDoc.data().is_current === true || String(ayDoc.data().is_current) === "1";
  if (!isYearCurrent) {
    throw new Error("You can only generate charges for the current academic year. This structure is set to a year that is not currently active. Go to Academic Years, edit this year and tick 'Current Year'.");
  }
